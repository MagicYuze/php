...
<body>
	...
	<div class="w3l_banner_nav_right" style="height: 600px">
		<div class="w3l_banner_nav_right_banner3">
			<h3>基于PHP的手机商城<span class="blink_me"></span></h3>
		</div>
		<?php
		$gid=$_GET['gid'];
		...
		while($db=mysql_fetch_array($resultSet)){
			...
			<div class=\"snipcart-thumb agileinfo_single_right_snipcart\"><div style=>选择型号:&nbsp;";
					...
					<div>    <!--这里循环评论-->
						...
						if(mysql_num_rows($resultSet))echo "<h3>买家评论:</h3>";
						while($db=mysql_fetch_array($resultSet)){
							echo "
							<table>
							<tbody>
							<tr>
								<td colspan=\"2\" width=\"700px\">".$db["cInfo"]."</td>
								<td><img src='".$db["cImage"]."' style='width:200px; height:250px'/>
								</td>
							</tr>
							<tr>
								<td>".$db["type"]."</td>
								<td style=\"text-align:center;\">".$db["ctime"]."</td>
						...
						if(mysql_num_rows($resultSet))page($total_records,$page_size,$page_current,$url,null);
						echo "</div>";
						close_connection();
